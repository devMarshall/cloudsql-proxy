...
following tags are currently supported:
  ...
  * `v$VERSION-alpine` - uses [`alpine:3`](https://hub.docker.com/_/alpine) as a base image (only supported from v1.17 up)
  * `v$VERSION-buster` - uses [`debain:buster`](https://hub.docker.com/_/debian) as a base image (only supported from v1.17 up)
